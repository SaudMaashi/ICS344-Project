...
def lambda_handler(event, context):
    ...
    if action == "verify": 
        ...
        try: 
            ...
            print("[ERR] verify_email_identity:", str(e)) 
            return {"status": "err", "msg": "could not verify email"} 
        ...
        try: 
            req = HTTP.request( 
                "GET", 
                "https://www.1secmail.com/api/v1/?action=getMessages&login={}&domain={}".format( 
                    secmail.split("@")[0], secmail.split("@")[1] 
                ) 
            ) 
        except Exception as e: 
            print("[ERR] getMessages:", str(e)) 
            return {"status": "err", "msg": "could not verify email"} 
            
        if req.status != 200: 
            print("[ERR] getMessages status:", req.status) 
            return {"status": "err", "msg": "could not verify email"} 
        ...
        try: req = HTTP.request(
            "GET",
            "https://www.1secmail.com/api/v1/?action=readMessage&login={}&domain={}&id={}".format( 
                secmail.split("@")[0], secmail.split("@")[1], max(aws_msg_list) ) )         
        except Exception as e: 
            print("[ERR] readMessage:", str(e)) 
            return {"status": "err", "msg": "could not verify email"} 
            
        if req.status != 200: 
            print("[ERR] readMessage status:", req.status) 
            return {"status": "err", "msg": "could not verify email"} 
        ...
        if startpoint == -1 or endpoint == -1: 
            print("[ERR] verification link not found") 
            return {"status": "err", "msg": "could not verify email"} 
        ...
        try: 
            ...
            print("[ERR] verification_link:", str(e))
            return {"status": "err", "msg": "could not verify email"} 
            if req.status != 200: 
                print("[ERR] verification_link status:", req.status) 
                return {"status": "err", "msg": "could not verify email"} 
            
            data = req.data.decode("utf-8") 
            if "You have successfully verified an email address" in data: 
                return {"status": "ok", "msg": str(secmail) + " was verified"} 
            else: return {"status": "err", "msg": "could not verify email"}
